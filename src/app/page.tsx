...
import { rnd, shuffle } from "@/lib/obfuscate";
...
export default function HomePage() {
  ...
  const categoriesSection = (
    <section key="categories" className="flex flex-col gap-6">
  ...
  const featuredSection = (
    <section key="featured" className="flex flex-col gap-6">
      <div className="flex items-end justify-between gap-4">
        <div>
  ...
  const makersBanner = (
    <section key="makers" className="relative overflow-hidden rounded-2xl bg-emerald-900 text-white">
  ...
  const newArrivalsSection = (
    <section key="new" className="flex flex-col gap-6">
      <div className="flex items-end justify-between gap-4">
        <div>
  ...
  const valueProps = <ValueProps key="vp" />;
  const testimonials = <Testimonials key="tst" />;
  const newsletter = <Newsletter key="nl" />;

  // Optional sections: each independently coin-flipped per build.
  const optional = [
    { key: "categories", node: categoriesSection, alwaysShow: true },
    { key: "featured", node: featuredSection, alwaysShow: true },
    { key: "vp", node: valueProps, alwaysShow: false },
    { key: "makers", node: makersBanner, alwaysShow: false },
    { key: "new", node: newArrivalsSection, alwaysShow: false },
    { key: "tst", node: testimonials, alwaysShow: false },
    { key: "nl", node: newsletter, alwaysShow: true },
  ];
  const visible = optional.filter(
    (s) => s.alwaysShow || rnd("home-section-show::" + s.key) > 0.4,
  );
  const order = shuffle(visible, "home-section-order");
